...
public class Button {
	...
	public void CheckIsClicked(float x, float y) {
		if (x + (Main.getScreenOffsetx()) > X && x < X + Width + Main.getScreenOffsetx() && y > Y + Main.getScreenOffsety() && y < Y + Height + Main.getScreenOffsety())
			Clicked = true;
		else
			Clicked = false;
	}
	...
	public void CheckIsHovered(float x, float y) {
		if (x > X + (Main.getScreenOffsetx() / 2) && x < X + Width + (Main.getScreenOffsetx() * 1.5) && y > Y + Main.getScreenOffsety() && y < Y + Height + Main.getScreenOffsety())
			Hovered = true;
		else
			Hovered = false;
	}
