...
public class HappyPerson extends Mob {
	private Sprite sprite;
	private int anim = 0;
	private String Char;

	public HappyPerson(float x, float y, String cHar) {
		setX(x);
		setY(y);
		Char = cHar;
		setDir(1);
	...
	public void Speak(Graphics g) {
		while (SpeakTimer <= 1000) {
			SpeakTimer++;
			g.drawString("Yay Im a real boy!", (int) (this.getX() - 30), (int) (this.getY() + 25));
		}
		if (SpeakTimer > 1000) {
			SpeakTimer = 0;
			Speaking = false;
